...
class MstockWebSocket:
    ...
    def parse_binary_packet(data: bytes) -> Optional[Dict]:
        ...
        try:
            ...
            # Check if data has the 4-byte header or is just the 379-byte packet
                ...
                logger.error(f"Invalid packet size: {len(data)} bytes (expected 51, 123, 379 or 383+)")
                return None
        ...
        try:
            ...
            async with websockets.connect(self.ws_url, ping_interval=None) as websocket:
                ...
                for attempt in range(max_attempts):
                    try:
                        response = await asyncio.wait_for(websocket.recv(), timeout=2.0)  # Reduced from 5.0

                        if isinstance(response, bytes):
                            logger.debug(f"Received binary data of {len(response)} bytes")

                            # Check if this is a quote packet (51, 123, 379, or 383+ bytes)
                            if len(response) == 51 or len(response) == 123 or len(response) == 379 or len(response) >= 383:
                        ...
                        logger.debug(f"Timeout waiting for response (attempt {attempt + 1}/{max_attempts})")
                        if attempt == max_attempts - 1:
                            logger.info("No quote data received (market may be closed or symbol unavailable)")
                            return None
    ...
    async def connect_stream_async(self, data_callback):
        ...
        try:
            ...
            async with websockets.connect(self.ws_url, ping_interval=20, ping_timeout=10) as websocket:
                ...
                try:
                    login_response = await asyncio.wait_for(websocket.recv(), timeout=3.0)
                    logger.debug(f"Login response: {login_response if isinstance(login_response, str) else 'binary'}")
                except asyncio.TimeoutError:
    ...
    async def subscribe_stream_async(self, correlation_id: str, token: str, exchange_type: int, mode: int):
        """
        ...
        try:
            ...
            unsubscribe_msg = {
                ...
                "params": {
                    "mode": sub['mode'],
                    "tokenList": [{
                        "exchangeType": sub['exchange_type'],
                        "tokens": [str(sub['token'])]
                    }]
                }
            }
