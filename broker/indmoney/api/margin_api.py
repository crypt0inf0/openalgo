...
def calculate_margin_api(positions, auth):
    ...
    for position in transformed_positions:
        try:
            ...
            try:
                ...
                logger.error(f"Failed to parse JSON response for {position.get('securityID')}: {response.text}")
                failed_positions.append(position.get('securityID'))
                continue

            logger.info(f"Margin calculation response for {position.get('securityID')}: {response_data}")
            ...
            if standardized_response.get('status') == 'success':
                ...
                failed_positions.append(position.get('securityID'))
                logger.warning(f"Failed to calculate margin for {position.get('securityID')}: {standardized_response.get('message')}")
    ...
    logger.info(f"Aggregated margin calculation completed. Success: {successful_count}/{len(transformed_positions)}")
